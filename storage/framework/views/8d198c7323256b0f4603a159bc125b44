...
<?php
    ...
    $bidder = array();   // mảng thông tin bên mời thầu
       ...
       /******************* Hang hoa **********************/
        ...
        foreach ( $content = $goithau->find("tr td[align=left] a")  as  $value) {
        ...
        //}
        foreach($content = $goithau->find("tr[bgcolor]") as $value){
            //$bidder_temp =  $value->children(2)->innertext();
//          $bidder_temp =  $value->innertext();
            $vl = $value->find("td[align=center]",0)->innertext;  //Truyen so 0 phia sau => du lieu se tra ve kieu chuoi (neu ko truyen so 0, du lieu tra ve kieu mang)
            $bidder_temp = $vl;
            array_push($bidder, $bidder_temp);
        }
        ...
        /******************* Xay lap *******************/
            ...
            foreach($content = $xaylap->find("tr td[align=left] a") as $value) {
              if (substr( $value->attr['onclick'],0,13)=='goAspTuchLive')
                   {// Lấy dữ liệu ID_NO với goAspTuchLive
                    $id_no = substr( $value->attr['onclick'],30,2);
                    $bid_no =substr( $value ->attr['onclick'], 15 ,11);
             ...
             // Tìm thông tin bên mời thầu
          foreach($content = $xaylap->find("tr[bgcolor]") as $value){
              //$bidder_temp =  $value->children(2)->innertext();
  //          $bidder_temp =  $value->innertext();
              $vl = $value->find("td[align=center]",0)->innertext;  //Truyen so 0 phia sau => du lieu se tra ve kieu chuoi (neu ko truyen so 0, du lieu tra ve kieu mang)
              $bidder_temp = $vl;
          ...
          //array_shift($bidder);
           // Thêm dữ liệu
            ...
            $bidder = array();   // mảng thông tin bên mời thầu
             ...
             // Lấy dữ liệu tên gói thầu
            foreach($content = $tuvan->find("tr td[align=left] a") as $value) {
              if (substr( $value->attr['onclick'],0,13)=='goAspTuchLive')
                   {// Lấy dữ liệu ID_NO với goAspTuchLive
                    $id_no = substr( $value->attr['onclick'],30,2);
                    $bid_no =substr( $value ->attr['onclick'], 15 ,11);
             ...
             // Tìm thông tin bên mời thầu
          foreach($content = $tuvan->find("tr[bgcolor]") as $value){
              //$bidder_temp =  $value->children(2)->innertext();
  //          $bidder_temp =  $value->innertext();
              $vl = $value->find("td[align=center]",0)->innertext;  //Truyen so 0 phia sau => du lieu se tra ve kieu chuoi (neu ko truyen so 0, du lieu tra ve kieu mang)
              $bidder_temp = $vl;
          ...
          //array_shift($bidder);
           // Thêm dữ liệu
            ...
            $bidder = array();   // mảng thông tin bên mời thầu
             ...
             // Lấy dữ liệu tên gói thầu
            foreach($content = $phituvan->find("tr td[align=left] a") as $value){
              if (substr( $value->attr['onclick'],0,13)=='goAspTuchLive')
                   {// Lấy dữ liệu ID_NO với goAspTuchLive
                    $id_no = substr( $value->attr['onclick'],30,2);
                    $bid_no =substr( $value ->attr['onclick'], 15 ,11);
             ...
             // Tìm thông tin bên mời thầu
          foreach($content = $phituvan->find("tr[bgcolor]") as $value){
              //$bidder_temp =  $value->children(2)->innertext();
  //          $bidder_temp =  $value->innertext();
              $vl = $value->find("td[align=center]",0)->innertext;  //Truyen so 0 phia sau => du lieu se tra ve kieu chuoi (neu ko truyen so 0, du lieu tra ve kieu mang)
              $bidder_temp = $vl;
         ...
         // Xóa phần tử thừa trong mảng người mời thầu
          //array_shift($bidder);
           // Thêm dữ liệu
            ...
            // Lấy dữ liệu tên gói thầu
            foreach($content = $honhop->find("tr td[align=left] a") as $value){
              if (substr( $value->attr['onclick'],0,13)=='goAspTuchLive')
                   {// Lấy dữ liệu ID_NO với goAspTuchLive
                    $id_no = substr( $value->attr['onclick'],30,2);
                    $bid_no =substr( $value ->attr['onclick'], 15 ,11);
             ...
             // Tìm thông tin bên mời thầu
          foreach($content = $honhop->find("tr[bgcolor]") as $value){
              //$bidder_temp =  $value->children(2)->innertext();
  //          $bidder_temp =  $value->innertext();
              $vl = $value->find("td[align=center]",0)->innertext;  //Truyen so 0 phia sau => du lieu se tra ve kieu chuoi (neu ko truyen so 0, du lieu tra ve kieu mang)
              $bidder_temp = $vl;
          ...
          //array_shift($bidder);
           // Thêm dữ liệu
            ...
            $bidder = array();   // mảng thông tin bên mời thầu
             ...
             // Lấy dữ liệu tên gói thầu
            foreach($content = $luachonnhadautu->find("tr td[align=left] a") as $value){
              if (substr( $value->attr['onclick'],0,13)=='goAspTuchLive')
                   {// Lấy dữ liệu ID_NO với goAspTuchLive
                    $id_no = substr( $value->attr['onclick'],30,2);
                    $bid_no =substr( $value ->attr['onclick'], 15 ,11);
             ...
             // Tìm thông tin bên mời thầu
          foreach($content = $luachonnhadautu->find("tr[bgcolor]") as $value){
              //$bidder_temp =  $value->children(2)->innertext();
  //          $bidder_temp =  $value->innertext();
              $vl = $value->find("td[align=center]",0)->innertext;  //Truyen so 0 phia sau => du lieu se tra ve kieu chuoi (neu ko truyen so 0, du lieu tra ve kieu mang)
              $bidder_temp = $vl;
